...
import { BrowserIdleService } from '../../../ng-service/browser.idle.service';
@Component({
  templateUrl: './market.replay.component.html',
  styleUrls: ['./market.repay.component.css']
})
...
export class MarketReplayComponent implements OnInit, OnDestroy {
  ...
  public gridOptionsPriceRange:GridOptions;
  ...
  */
  ...
  public lcq: number;
  public luq: number;
  public awfromltp: number;
  public slfromltp: number;
  public enaUp: boolean = false;
  public enaDown: boolean = false;
  public enaplay: boolean = false;
  /**
  ...
  constructor(private dataShareService: DataShareService, private router: Router, private route: ActivatedRoute,
    private restService: RestService, private messageService: MessageService, public genericComponent: GenericComponent, public webSocketService: WebSocketService, public graphService: GraphService, public cookieService: CookieService, public browserIdleService: BrowserIdleService) {
    ...
    this.gridOptionsPriceRange={
      rowData: [],
      rowHeight: 19,
      headerHeight: 17,
      groupHeaderHeight:20, 
      columnDefs: thisConstants.ColumnDefs.PriceRange,
      suppressColumnVirtualisation: true,      
    }
    this.rowClassRules = {
  ...
  ngOnInit() {
    ...
    if (headerData != undefined) {
      ...
      this.luq = headerData.mrReqLargeUnexecutedQty;
      this.lcq = headerData.mrReqLargeCxlQty;
      this.awfromltp = headerData.mrReqPectAwayLTP;
      this.slfromltp = headerData.mrReqSLPectAwayLTP;
    ...
    this.highlightForm = new FormGroup({
      ...
      mkwhgOrderNoArray: new FormControl(),
    ...
    this.gotoForm = new FormGroup({
      ...
      frmbuypanno: new FormControl('', [Validators.maxLength(10), Validators.pattern(thisConstants.patterns.alphanumeric), Validators.pattern(thisConstants.patterns.pan)]),
      frmsellerpanno: new FormControl('', [Validators.maxLength(10), Validators.pattern(thisConstants.patterns.alphanumeric), Validators.pattern(thisConstants.patterns.pan)]),
      frmbrokereno: new FormControl('', [Validators.pattern(thisConstants.patterns.alphanumeric), Validators.maxLength(5)]),
    });
  }
        ...
        if (this.highlightForm != undefined && this.highlightForm != null) {
          if (this.highlightForm.value != undefined && this.highlightForm.value != null) {
            ...
            */
            ...
            /**
            * Highlighter on Order Number
            */
            if (this.highlightForm.value.mkwhgOrderNoArray != null) {
              if (data[i].mrBuyOrderNmbr != null) {
                if (this.highlightForm.value.mkwhgOrderNoArray.indexOf(data[i].mrBuyOrderNmbr.toString()) >= 0) {
                  data[i].mrHighlighter = 'H';
                  continue;
                }
              }
              if (data[i].mrSellOrderNmbr != null) {
                if (this.highlightForm.value.mkwhgOrderNoArray.indexOf(data[i].mrSellOrderNmbr.toString()) >= 0) {
                  data[i].mrHighlighter = 'H';
                  continue;
                }
              }
  ...
  pauseButtonClickEvent() {
    ...
    this.browserIdleService.keepWatch();
  }
  ...
  refreshButtonClickEvent(gotoClick) {
    ...
    this.webSocketService.isLoadedResponse = false;
    this.mdAwayLtpData = {};
    this.orderTradeLogsData = '';

    (this.gridOptionsEventLog.api ? this.gridOptionsEventLog.api.setRowData([]) : '');
    (this.gridOptionsActivityLog.api ? this.gridOptionsActivityLog.api.setRowData([]) : '');
    (this.gridOptionsBuyOrderLog.api ? this.gridOptionsBuyOrderLog.api.setRowData([]) : '');
    (this.gridOptionsSellOrderLog.api ? this.gridOptionsSellOrderLog.api.setRowData([]) : '');
    (this.gridOptionsPriceRange.api ? this.gridOptionsPriceRange.api.setRowData([]) : '');
  ...
  goToButtonClickEvent() {
    ...
    var jsonObject = {
      ...
      mrBuyPan: tempObj.frmbuypanno,
      mrSellPan: tempObj.frmsellerpanno,
      mrBroker: tempObj.frmbrokereno,
  ...
  onAddOrderNoValueEvent(event) {
    let orderNo = event.value;
    let length = this.highlightForm.value.mkwhgOrderNoArray.length - 1;
    if (length > 0) {
      for (let i = 0; i < length; i++) {
        if (this.highlightForm.value.mkwhgOrderNoArray[i].toUpperCase() == orderNo.toUpperCase()) {
          this.highlightForm.value.mkwhgOrderNoArray.pop();
          return;
        }
      }
    }
    if (!orderNo.match(thisConstants.patterns.numeric) || orderNo.length > 20) {
      this.highlightForm.value.mkwhgOrderNoArray.pop();
      this.genericComponent.showErrorMessage(MessageConstant.ValidationMessage.OrderNo);
    }
  }
  ...
  marketReplayPopupGridDataCount(type, params) {
    ...
    if (type == 'BUY') {
      ...
      this.restService.startPostRequestWithParamater('marketReplayPopupGridDataCount', this.jsonData).subscribe(data => { this.marketReplayPopupGridDataCountCallBack(data) });
    }
    if (type == 'SELL') {
      this.BStype = "SELL";
      this.jsonData = {
    ...
    if (type == 'BUYLTP') {
      this.BStype = "BUYLTP";
      var mrTp = this.orderBook.mrtradeLogData.split(",")[7];
      mrTp = mrTp.split(":")[1];
      mrTp = mrTp.trim();
      ...
      this.jsonData = {
        ...
        mrSeqNmbr: this.orderBook.mrSeqNmbr,
        mrStartSeqNmbr: 0,
        limitPrice: (mrTp) * (1 - (mrReqPectAwayLTP / 100)),
        thresholdQty: 0,
        thresholdCxlQty: 0,
      };
    ...
    if (type == 'SELLLTP') {
      this.BStype = "SELLLTP";
      var mrTp = this.orderBook.mrtradeLogData.split(",")[8];
      mrTp = mrTp.split(":")[1];
      mrTp = mrTp.trim();
      var mrReqPectAwayLTP = this.dataShareService.getMwscrHeaderData().mrReqPectAwayLTP;
      this.jsonData = {
    ...
    if (type == 'BUYLTPSL') {
      ...
      this.jsonData = {
        ...
        mrSeqNmbr: this.orderBook.mrSeqNmbr,
        mrStartSeqNmbr: 0,
        limitPrice: (mrTp) * (1 + (mrReqSLPectAwayLTP / 100)),
        thresholdQty: 0,
        thresholdCxlQty: 0,
      };
  ...
  marketReplayPopupGridData(pageNum: number) {
    try {
      ...
      this.paginationData = {};
      this.restService.stopPostRequestWithParamater('marketReplayPopupGridData', this.jsonData).subscribe(data => { this.marketReplayPopupGridDataCallBack(data, pageNum, this.BStype) });
    } catch (err) {
      this.genericComponent.showErrorMessage(survConst.errorMsg.noDataErr);
    }
  }
  ...
  playMarketReplayScocket() {
    ...
    this.webSocketService.forwardBackWardFlag = false;
    this.webSocketService.forwardManualFlag = true;
    this.gotoForm.disabled;
    this.browserIdleService.keepUnWatch();
  ...
  DisabledGoto(event) {
    if (this.gotoForm && this.gotoForm.controls) {
      let tmp;
      for (var i = 0; i < event.target.attributes.length; i++) {
        if (event.target.attributes[i].nodeName == "formcontrolname") {
          tmp = i;
        }
      }
      Object.keys(this.gotoForm.controls).forEach(formkey => {
        if (event.target.attributes[tmp].nodeValue && this.gotoForm.controls[event.target.attributes[tmp].nodeValue] && this.gotoForm.controls[event.target.attributes[tmp].nodeValue].value && this.gotoForm.controls[event.target.attributes[tmp].nodeValue].value.length > 0) {
          if (formkey != event.target.attributes[tmp].nodeValue) {
  ...
  getJsonForStart() {
    ...
    var lastSeqNum = 0;
    if (!this.webSocketService.forwardBackWardFlag) {
      lastSeqNum = this.webSocketService.activityArr.length != 0 ? this.webSocketService.activityArr[this.webSocketService.activityArr.length - 1].mrSeqNmbr : 0;
    } else {
      lastSeqNum = this.webSocketService.activityArr.length != 0 ? this.webSocketService.activityArr[0].mrSeqNmbr : 0;
    }
    jsonObject = {
      mrRunId: this.runId,
      mrSeqNmbr: this.mrSeqNmbr,
  ...
  /* for creating buysell price range array */
  buySellpriceRange(arr) {
    let gridData:any=[];
    let tempobj:any={};          
    var mrTp = arr.mrtradeLogData.split(",")[8];
    mrTp = Number(mrTp.split(":")[1].trim());          
    if(isNaN(mrTp)) {
     mrTp=0;
    }           
    for(let c=0;c<4;c++) {
         ...
         switch(c) {
           case 0: {
             tempobj.buyQuanty=arr.mrBuyQtyAwayLTPRange1;
             tempobj.sellQuanty=arr.mrSellQtyAwayLTPRange1;
             break;
           }
           case 1: {
             tempobj.buyQuanty=arr.mrBuyQtyAwayLTPRange2;
             tempobj.sellQuanty=arr.mrSellQtyAwayLTPRange2;
             break;
           }
           case 2: {
             tempobj.buyQuanty=arr.mrBuyQtyAwayLTPRange3;
             tempobj.sellQuanty=arr.mrSellQtyAwayLTPRange3;
             break;
           }
           case 3: {
             tempobj.buyQuanty=arr.mrBuyQtyAwayLTPRange4;
             tempobj.sellQuanty=arr.mrSellQtyAwayLTPRange4;
             break;
           }
         }      
         tempobj.priceRange=thisConstants.BUY_SELL_PRICE_RANGE_LBL[c];
         tempobj.buyFromRange= (c==0) ? "-":gridData[c-1].buyToRange;
         tempobj.buyToRange=(c==3 ? "-": (mrTp-(mrTp*(thisConstants.BUY_SELL_PRICE_RANGE[c] / 100))).toFixed(2));       
         tempobj.sellFromRange=(c==0) ? "-":gridData[c-1].sellToRange;
         tempobj.sellToRange=(c==3 ? "-": (mrTp+(mrTp*(thisConstants.BUY_SELL_PRICE_RANGE[c] / 100))).toFixed(2));        
