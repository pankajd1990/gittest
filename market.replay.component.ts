...
export class MarketReplayComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  /* flag added for enable disable play and fwdbwd btn */
  public isRefresh:Boolean=false;
  public enBlPlayButton:boolean=true;
  public enbfwd:boolean=true;
  public enbbkd:boolean=true;
  public lastSeqnum:number=0;
  /*/
  /**
  ...
  backwardButtonClickEvent() {
    ...
    if (this.playPauseButton) {
       ...
       if(currow && currow[0] && currow[0].rowIndex==0){
          // this.webSocketService.forwardBackWardFlag = true;
          let rows = this.gridApiActivityLog.getSelectedRows();             
          if(rows && rows[0] && rows[0].mrSeqNmbr==this.mrSeqNmbr){
             this.enbbkd=false;
          }
          else{
            this.enbbkd=true;
            this.gridApiActivityLog.ensureIndexVisible(0);
  ...
  forwardButtonClickEvent() {
    if (this.playPauseButton) {
       ...
       if(currow && currow[0] && currow[0].rowIndex < last_display_row ){
         ...
         // this.webSocketService.forwardBackWardFlag = false;
         let rows = this.gridApiActivityLog.getSelectedRows();  
         if(rows && rows[0] && rows[0].mrSeqNmbr==this.lastSeqnum){
            this.enbfwd=false;
         }
         else{
             this.enbfwd=true;
